...
for(j in 1:3){
  ...
  for(rep in 1:rep0)
  {
    ...
    out1 <- fitting(y, X, BS1, cliques=clique.list, Total_itr=30, id, Als)
...
err.median = apply(err[1:40,,], 2:3, median)
err.mean = apply(err[1:40,,], 2:3, mean)
err = round(rbind(err.median,err.mean),3)
colnames(err) = c("sd=0.5","sd=1","sd=2")
rownames(err) = c("median RS","median URS","mean RS","mean URS")
err
#write.csv(err,"Bellshaped_spline_clique15_sd1_2_5.csv")
...
#ggsave('Bellshaped_spline_clique15_sd1_2_5.png', p1, width = 17,height=11, dpi = 300)
