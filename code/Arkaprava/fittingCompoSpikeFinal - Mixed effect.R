fitting <- function(y, X=Delta, BS1, cliques=clique.list, Total_itr=1000){
  K <- length(cliques)
  #n <- nrow(X)
  p <- ncol(X)
  ...
  for(itr in 1:Total_itr){
    ...
    for(i in 1:length(cliques)){
      ...
      p1 <- 1-p0
      pstar   <- p1*dnorm(bi, sd = sqrt(v1))/(p1*dnorm(bi, sd = sqrt(v1))+p0*dnorm(bi, sd = sqrt(v0)))
  ...
  return(list(gamma=gamma,err=mean((ftf - BS1%*%Delta %*% b)^2), esti=BS1 %*% Delta %*% b))
