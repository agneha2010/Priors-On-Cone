...
result = cbind(result1,out4$esti)
colnames(result)[5] = "RS"
write.csv(result , "result_day1.csv")
library(dplyr)
library(reshape2)
library(ggplot2)
...
p1  
#ggsave('ydata1_itr100.png', p1, width = 17,height=11, dpi = 300)

par(mfrow=c(1,1))
plot(out1$gamma)
par(mfrow=c(2,1))
plot(out1$esti,type="l",ylim=c(0,355))
points(colMeans(ydata3))

plot(out2$esti,type="l",ylim=c(0,355))
points(colMeans(ydata3))

plot(out3$esti,type="l",ylim=c(0,355))
points(colMeans(ydata3))
