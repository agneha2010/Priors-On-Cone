fittingUN <- function(y, xt, Ti, splin=T, Total_itr=1000){
  ...
  for(itr in 1:Total_itr){
    ...
    #ind <- which(dvec>0.01) 
    #}
    M <- solve(crossprod(X1[,ind]) + sig^2*diag(J)/100)%*%crossprod(X1[,ind], yred)#diag(1/dvec) - diag(1/dvec) %*% t(X) %*% solve(diag(n)+ X1%*%diag(1/dvec)%*%t(X)) %*% X %*% diag(1/dvec)
  ...
  return(list(err=mean((ftf - BS1 %*% b)^2), esti=BS1 %*% b))
